...
//
// Requests go through the PropFirm backend (VITE_PROPFIRM_API_URL +
// /bi2x-chart/*), not the feed's own domain directly — that server sends no
...
// CORS-blocked (observed live from this app's origin too). The PropFirm
// backend carries the identical passthrough proxy (internal/api/bi2xchart.go,
// ported from Dex-Backend's), keeping the app's rule that the browser only
// ever talks to its own backend.
//
...
const BI2X_CHART_BASE_URL = (import.meta.env.VITE_PROPFIRM_API_URL ?? "http://localhost:8090") + "/bi2x-chart";
